...
def display_playlist(iterable, name):
...
songs_list = delete_occurrences(songs_list)


### CLASS PLAYLIST
class Playlist(DynamicArray):
    def __init__(self, name="Playlist", owner=None):
        super().__init__()
        self.name = name
        self.owner = owner
    
    def __repr__(self) -> str:
        songs_titles = [str(elem) for elem in self]
        songs_string = '\n'.join(songs_titles)
        return f"{self.name}\nby {self.owner}\n\n{songs_string}"
...
class User:
    ...
    def __init__(self, name: str) -> None:
        ...
        self.playlists = DynamicArray()
        self.playlists.append(self.liked_songs)
    
    def add_to_followings(self, artist_name: str) -> None:
        if artist_name not in self.following:
            self.following.append(artist_name)

    def get_playlist_by_name(self, playlist_name: str) -> Playlist:
    ...
    def create_playlist(self, name) -> DynamicArray:
        if name == None:
            count += 1
